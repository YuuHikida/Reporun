...
namespace ReportSystem.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        {
            /*_contextはDbContextインスタンス、.projectはその中のテーブル
             FindAsync(projectSelect)は*/
            ...
            if (ModelState.IsValid)
            {
                ...
                Debug.WriteLine("-----------------------------------------------------");
                Debug.WriteLine($"Afteruser.icon:{user.Icon}");
        ...
        public bool DuplicateNewRoles(InputModel Input)
            {
            
            if (Input.NewRole == "社長" || Input.NewRole == "会長")
            {
                List<string> CheackStr = _context.user.Select(u => u.NewRole).ToList();
                //string CheackStr = await _context.user.FindAsync(user2.NewRole);
                //var Checkstr = await _context.user.Where(u => u.NewRole == "社長" || u.NewRole == "会長");
                for (int i = 0; i < CheackStr.Count; i++)
                {
                    if (CheackStr[i] == Input.NewRole)
                    {
                        //return Redirect((string)TempData["CastamError"]);
                        return true;
                    }
                }
                /*
                    if (CheackStr.Contains("社長") || CheackStr.Contains("会長"))
                    {
                        //ここで画面にerrorを返す
                        TempData["CastamError"] = "社長、及び会長は既に役割が割り振られています。";
                        return Redirect((string)TempData["CastamError"]);
                    }
                    else
                    {
